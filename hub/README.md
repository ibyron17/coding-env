...
- 프라이버시 우선 설계 — 프롬프트 발췌 상한·보관 기간·수집 항목을 옵트인/옵트아웃으로
  직접 제어한다
- 탭이 보이는 동안만 화면을 갱신하고 숨겨지면 조용히 멈춘다 — 서버의 5초 수집 루프는
  별개로 계속 돈다
- 카드를 드래그해 순서를 직접 정할 수 있고, 클릭하면 그 프로젝트의 대시보드를 그 자리에서
  본다
- 세션(5시간)·주간(7일) 사용량 한도를 패널 하나로 함께 보여준다(옵트인)
...
| 훅 | 세션 이벤트를 모으는 전역 훅 6종 옵트인 설치·제거 |
| 상태줄 | 터미널에 사용량 한 줄 표시 옵트인 |
| 진단 | 훅·이벤트·서버·사용량 상태를 한 번에 점검 |

![통합 허브 다크 테마 예시 — 프로젝트 카드·사용량 패널](../docs/images/hub-sample-dark.png)
...
`~/.claude/hub/bin/` 에 15개 파일을 설치한다. `--scope` 인자가 없다 — 설치 위치는 이 하나뿐이며,
...
/hub install                 # 2. 전역 훅 6개 + 상태줄 등록(옵트인)
...
- **서버의 수집 주기(5초)와 브라우저의 폴링 주기(60초)는 별개다.** 페이지는 1분마다 스스로
  ...
  **탭이 보이지 않는 동안은 이 1분 타이머가 폴링 자체를 건너뛴다**(`document.visibilityState`
  기준, 사용자 결정 — 안 보는 화면을 갱신할 이유가 없다는 판단). 창 활성화·가시성 전환·
  새로고침 버튼은 이 게이트와 무관하게 항상 폴링한다 — 새로 시작된 세션을 잡는 경로이기
  때문이다.
...
- **크래시하면 그대로 죽는다.** 자동으로 되살리지 않는다. 대신 `/hub server status` 의
  ...
  **브라우저 채널은 폴링이 도는 동안(탭이 보이는 동안)만 나타난다** — 탭이 숨겨져 자동
  폴링이 멈춰 있으면, 창 활성화·가시성 전환·새로고침으로 폴링을 한 번 일으켜야
  "서버 연결 끊김"을 확인할 수 있다.
...
- 프롬프트 앞부분 **500자**가 `~/.claude/hub/events/*.jsonl` 에 평문으로 최대 **7일** 보관된다.
  `config.json` 의 `record_prompt_excerpt: false` 로 끌 수 있다.
  (카드에 보이는 것은 앞 120자뿐이고, 나머지는 발췌에 마우스를 올렸을 때 뜨는 툴팁이 보여준다 —
  즉 이 500자는 **화면에 표시할 목적으로** 보관되는 분량이다. 상한을 낮추려면
  `hub/bin/hub_hook.py` 의 `PROMPT_EXCERPT_MAX_CHARS` 를 고치고 다시 설치한다.)
- **CLI 가 자동으로 덧붙이는 블록은 기록하지 않는다.** 훅이 받는 프롬프트에는 사용자가 타이핑한
  것 외에 `<ide_selection>`·`<ide_opened_file>`(편집기 선택·열린 파일)과
  `<task-notification>`(서브에이전트 완료 알림)이 앞에 붙어 올 수 있다. 이 선행 블록은 벗겨내고
  남은 실제 입력만 기록하며, 벗기고 남은 것이 없으면(순수 알림) **아무것도 기록하지 않는다.**
  그래서 선택한 소스코드나 알림 원문이 이벤트 로그에 남지 않는다 — 실측 187건 기준으로 이
  제외 덕에 상한을 120자에서 500자로 올려도 저장 분량은 종전보다 오히려 줄어든다.
...
- **프로젝트 상세 패널(서버 모드)은 허브 오리진(`http://127.0.0.1:8794`)에서 실행된다.**
  카드를 클릭해 여는 패널의 iframe 은 그 프로젝트의 `.claude/dashboard.html` 을 원본 그대로
...
- 새로고침·테마 버튼은 제목(`h1`)과 같은 줄 오른쪽 끝에 있다 — 예전처럼 화면에 고정되지
  않고 스크롤하면 함께 올라간다. 테마 버튼은 라이트/다크 **2상태** 아이콘이다 — 첫
...
- 세션 줄에는 상태 배지·경과 시간과 함께, 그 세션에서 실행됐던 **서브에이전트** 타입 칩이
  ...
  칩으로도 툴팁으로도 나타나지 않는다.
- 세션 줄 아래 **발췌**는 그 세션에서 마지막으로 입력한 프롬프트다. 카드 높이가 고정이라
  앞 **120자**까지만 보이고, 그보다 길면 끝에 `…` 가 붙는다 — 잘린 뒷부분은 발췌에 마우스를
  올리면 뜨는 툴팁이 보여준다(기록 상한 500자까지. 그 상한에서 잘렸으면 툴팁 문구 끝에도
  `…` 가 남는다). 잘리지 않은 발췌에는 툴팁이 붙지 않는다.
  편집기 선택 블록이 앞에 붙어 온 프롬프트는 그 블록을 벗긴 실제 입력이 발췌가 되고, 서브
  에이전트 완료 알림처럼 사용자 입력이 없는 턴은 **직전 발췌를 그대로 유지한다** — 발췌는
  "마지막 이벤트"가 아니라 "마지막으로 **입력한** 프롬프트"다.
  화면 표시 단계에도 방어 필터가 하나 더 있다 — 자동주입 태그(`<ide_*>`·`<task-notification>`)로
  **시작하는** 발췌는 카드에서 숨긴다. 정상 데이터에는 훅이 이미 벗겨서 이 태그가 없어야
  하므로, 태그로 시작한다면 훅 필터 도입 전에 저장된 옛 기록(당시 120자 상한에서 잘려 닫는
  태그가 없다)일 가능성이 높다 — 그런 경우 벗기지 않고 통째로 숨긴다.
...
- **좀비 서브에이전트 가드**: `SubagentStop` 훅이 오지 않은 실행은(API 오류로 죽는 등) 시작
  후 **90분**이 지나면 "지금 돌고 있다는 근거"에서 빠진다 — 칩이 종료된 것과 똑같이 중립색으로
  바뀌고, 그 실행만으로 세션이 `작업중`을 유지하던 카드 glow 도 함께 꺼진다. 상태 배지와
  칩은 항상 같은 판정을 쓰므로, 카드는 `대기`인데 칩만 ● 로 남는 모순은 생기지 않는다.
...
- 지금 **작업중**인 세션이 전부 그 프로젝트의 대시보드 파일보다 **나중에** 시작됐으면, 티어 1
  줄 앞에 `이전 작업` 라벨이 붙고 그 세 줄의 톤이 낮아진다 — 새 작업을 시작했지만 아직
  `/dashboard init`(또는 `step`·`log`)을 하지 않은 프로젝트에서 며칠 전 진행률이 지금 것처럼
  보이는 것을 막는다. 세션이 조용해지면(대기·소식 없음) 라벨은 자연히 사라진다.
- 새로고침 버튼·프로젝트명·티어 배지·사용량 갱신 시각·서브에이전트 칩에 마우스를 올리거나
  Tab 으로 포커스하면 이 파일 안의 커스텀 **툴팁**이 뜬다 — 네이티브 title 툴팁의 지연 없이
  약 120ms 안에 나타나고, Escape·스크롤·클릭으로 즉시 닫힌다. 잘린 세션 발췌도 같은 툴팁을
  쓰지만, 발췌는 포커스를 받는 요소가 아니라 **마우스 hover 전용**이다(서브에이전트 칩과 같다).
...
**어느 카드든** 클릭하면 상세 패널이 열린다 — 티어 1 은 그 프로젝트의 `.claude/dashboard.html` 을,
나머지는 대시보드가 없는 사유를 보여준다.
...
- 티어 2·3 카드에는 대시보드가 없으므로 패널이 그 사유를 보여준다. `file://` 로 열었을
  때는 어떤 대시보드도 띄울 수 없어 모든 카드가 "허브 서버를 켜면…" 안내를 패널과 툴팁
  두 곳에서 같은 문구로 알린다.
- 패널 안 대시보드는 원본 화면과 똑같이 **1분마다 스스로 갱신**된다 — 허브 서버가 그 경로를
  서빙하는 것만으로 성립하며, 이 갱신 메커니즘 자체는 한 줄도 바뀌지 않는다.
- 패널 안에서는 원본 화면의 **플로팅(PiP) 버튼·안내 줄이 보이지 않는다** — 현재 생성기
  (`/dashboard init`)가 만드는 대시보드에는 패널용 숨김 CSS 가 이미 포함돼 있다. **구세대
  대시보드 파일**(플로팅 숨김 도입 이전에 생성된 것)에는 이 CSS 자체가 없어 예외적으로
  플로팅이 패널 안에서도 보일 수 있다 — 아래 「알려진 한계」 참조. 플로팅을 쓰려면 같은
  대시보드를 단독 탭(`http://localhost:{포트}/dashboard.html`)에서 열어야 한다. 이 단독 탭
  URL 은 `/dashboard init` 보고에 항상 함께 적힌다.
- 닫기 버튼과 ESC 두 가지로 닫는다. 닫으면 패널 안
  iframe 의 1분 폴링도 함께 멈춘다(숨은 요청이 남지 않는다).
- 넓은 화면에서는 목록이 왼쪽으로 밀리고 패널이 우측에서 미끄러져 들어온다. 닫으면 같은
  시간에 걸쳐 오른쪽으로 미끄러져 나간다 — 밀렸던 목록도 함께 제자리로 돌아온다. 목록은 그대로
  보이고 다른 카드를 눌러 내용을 바꿀 수 있다. 좁은 화면(1024px 미만)에서는 패널이 화면을 덮는다.
- **패널 안 대시보드는 허브 테마를 따라간다** — 같은 오리진의 `localStorage` 를 읽기 전용으로
  참조하므로, 허브에서 테마를 바꾸면 패널이 열려 있는 동안에도 즉시 반영된다.
- 지금 패널에 열려 있는 프로젝트의 카드는 배경색과 강조색 테두리로 표시된다 — 다른 카드를
  누르면 표시가 그 카드로 옮겨간다. 라이트 테마에서 배경색만으로는 페이지 배경과 구분되지
  않아(대비 1.04:1) 테두리를 함께 쓴다 — 면색을 더 어둡게 하는 쪽은 카드 안 본문 텍스트의
  대비 기준에 막힌다.

## 알려진 한계

- **구세대 대시보드의 플로팅 노출** — 커밋 `1e3d65a`(플로팅 숨김 도입) 이전에 생성된
  `.claude/dashboard.html` 파일에는 패널용 숨김 CSS 가 없어, 그 프로젝트의 상세 패널 안에서도
  원본 화면의 플로팅(PiP) 버튼·안내 줄이 보일 수 있다. 허브는 그 파일을 **읽기 전용으로만
  소비**하므로 직접 고치지 않는다 — 그 프로젝트에서 `/dashboard init`(또는 `step`·`log`)을
  한 번 더 실행하면 최신 생성기가 파일을 다시 써서 저절로 해소된다. 대시보드 파일보다
  **먼저 시작한** 세션이 계속 살아 있는 드문 경우에는 위 「이전 작업」 라벨도 함께 켜지지
  않으므로, 이 노출이 신호 없이 남을 수 있다.
- **`SubagentStop` 미발화의 근본 원인은 허브 밖이다** — 서브에이전트가 API 오류 등으로 죽으면
  CLI 가 그 훅을 아예 발화하지 않는 경우가 있다. 허브는 이벤트의 소비자일 뿐이라 발화를
  보장할 수단이 없다. 위 좀비 서브에이전트 가드는 그 흔적(90분 넘게 미종료)을 화면에서
  가려주는 **표시 가드**이며, 발화 누락 자체를 고치지는 않는다.
...
1. **`/hub statusline on`**(터미널) — Claude Code CLI 가 statusLine 명령에 주는 입력 JSON
   (`rate_limits.*.used_percentage`). 지금까지의 유일한 경로였다. **이 등록은 `/hub install`
   이 훅 설치에 이어 함께 처리한다**(옵트인 — "훅만"이라고 답하면 건너뛴다).
...
- 패널 안 툴팁은 만료 안내 줄(`조회되지 않음`) 하나뿐이다 — 왜 안 되는지(값 유효기간 만료)와
  어떻게 갱신되는지(세션 1회)를 알려준다. 캡처된 절대 시각을 보려면 `/hub status` 의
  `rate_limit_capture_age_ms`·`usage_sample_age_ms`(나이, ms)로 진단한다.
- 캡처 자체가 아예 없으면(statusLine 미등록·세션 미실행·스위치 꺼짐·계약 불일치·퍼센트가
  없는 구형 캡처) 패널 전체를 표시하지 않는다 — 부분 패널이나 "0%" 오해를 만들지 않는다.
  신규 사용자가 이 상태에 도달하지 않도록 `/hub install` 이 상태줄 등록까지 함께 처리한다.
- **한 번이라도 값이 캡처된 뒤에는, 세션을 한동안 안 돌려 캡처 나이가 5시간을 넘거나 세션
  (5시간) 창이 이미 리셋됐어도 패널은 사라지지 않는다** — 퍼센트 자리가 `—` 로 바뀌고
  하단에 `조회되지 않음 · 마지막 갱신 N시간 M분 전` 이 뜬다. 초기화 예정 시각 줄은 아직
  지나지 않았으면 그대로 보인다(절대 시각이라 지나기 전까지는 참이다). 새 세션을 한 번
  돌리면 퍼센트·막대가 자동으로 돌아온다.
...
- 캡처 자체가 없으면(statusLine 미등록·세션 미실행·구형 캡처 등) 패널 자체가 뜨지 않으므로,
  이 기능도 함께 보이지 않는다 — 초기화 시각은 기존 패널의 장식이지 별도 표시 조건이
  아니다. 세션(5시간) 창이 리셋된 경우는 패널이 사라지는 대신 위 「사용량 패널」 절의
  `조회되지 않음` 상태로 전환된다 — 세션 리셋 시각은 이미 지났으므로 그 줄만 자연히
  사라지고 주간 초기화 줄은 남는다.
...
```
~/.claude/hub/
├── bin/                        # hub/install.sh 가 배포. 15개 파일(hub_session.py·hub_project.py·
│                                #   hub_server_state.py 포함, docs/prps/hub-model-module-split.md)
├── config.json                 # 선택
