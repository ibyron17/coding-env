...
- **서버의 수집 주기(5초)와 브라우저의 폴링 주기(60초)는 별개다.** 페이지는 1분마다 스스로
  ...
  **작업중(●) 카드가 하나도 없으면 이 1분 타이머가 폴링 자체를 건너뛴다**(사용자 요청 —
  포커스·가시성 전환·새로고침으로 충분하다는 판단). 창 활성화·가시성 전환·새로고침 버튼은
  이 게이트와 무관하게 항상 폴링한다 — 새로 시작된 세션을 잡는 경로이기 때문이다.
...
- **크래시하면 그대로 죽는다.** 자동으로 되살리지 않는다. 대신 `/hub server status` 의
  ...
  **브라우저 채널은 폴링이 도는 동안(작업중 카드가 있을 때)만 나타난다** — 작업중 카드가
  없어 자동 폴링이 멈춰 있으면, 창 활성화·가시성 전환·새로고침으로 폴링을 한 번 일으켜야
  "서버 연결 끊김"을 확인할 수 있다.
