...
| 캡처 시각 | 줄의 `title` 속성에 `이 정보를 확인한 시각: 2026-08-12 09:14:03` (호버 툴팁) — **폐기됨(R5, [`hub-first-entry-and-ui-signals.md`](./hub-first-entry-and-ui-signals.md))**. 상대 시각만 남고, 절대 시각은 `/hub status` 의 `rate_limit_capture_age_ms` 로 진단한다 |
