...
**적용 범위**: 전체 경로(설계 → 구현 → 검수)와 축약 경로(구현 → 검수) 작업에 사용한다. 축약 경로도 구현↔검수 회귀가 여러 회차 반복될 수 있어 작업 추적 로그의 가치가 같다. 워크플로우 생략(로직 없는 수정)에는 쓰지 않는다.
...
> 전체 경로의 표준 단계 목록은 `"설계|승인|구현|검수"`, 축약 경로는 `"구현|검수"` 다.
...
| 구현 착수 — 세부 작업 목록이 확정됐을 때(선택) | `impl set "<작업1\|…>"` | 동일 |
| 서브에이전트를 디스패치하기 직전 | `impl <k> active ["담당·범위"]` | 동일 |
| 서브에이전트 결과 보고를 받은 직후 | `impl <k> done ["결과 요약"]` | 동일 |
| 구현 완료 | `step 3 done` + `log impl` | `step 1 done` + `log impl` |
| 검수 PASS / FAIL | `step 4 done|wait` + `log pass|fail` | `step 2 done|wait` + `log pass|fail` |
| 커밋·푸시 | `log commit` — 자동 발행이 띄운 로컬 서버를 함께 종료한다 | 동일 |

> 표준 단계 목록 — 전체 경로 = `설계|승인|구현|검수`, 축약 경로 = `"구현|검수"`. 커밋·푸시는 이 목록에 넣지 않는다(위 「호출 규약」 참조).
...
| `input[name="dzf"]` · `label[for^="dzf-"]` · `<style>` | **치환하지 않는다** | 라디오를 재삽입하면 사용자가 고른 유형 필터가 1분마다 초기화된다 |
...
파일과 달라지고, 라이브 비교 방식은 그 펼침을 1분마다 되접는다.
...
3. **자동 종료 — 첫 인자가 `commit` 일 때만 수행한다.**
   ...
   b. **종료** — Bash 1회. `{포트}` 는 a 를 통과한 숫자다.
          ...
          sleep 61 && pkill -f "http.server {포트} --bind 127.0.0.1"

      - **`sleep 61` 을 지우지 않는다.** 2단계가 방금 쓴 커밋 항목을 브라우저가 가져갈 시간이다
        (`POLL_INTERVAL_MS` = 60초 + 여유 1초). 이것이 없으면 서버가 먼저 죽어 플로팅 창이
      ...
      - 이 Bash 호출은 61초 동안 블로킹된다 — `log commit` 이 세션의 마지막 대시보드 호출이라
        감수한다. **폴링 주기(`POLL_INTERVAL_MS`)와 이 지연은 한 쌍이다** — 한쪽을 바꾸면
        반드시 다른 쪽도 함께 바꾼다.
...
| 티어 | URL | 조건 | 얻는 것 |
|------|-----|------|--------|
| 1 | `http://localhost:{포트}/dashboard.html` | 서버 재사용 또는 기동 성공 | 1분 폴링 자동 갱신 + 플로팅 버튼 활성(**단독 탭에서만** — 허브 상세 패널 안에서는 숨는다) |
...
| 티어 1 · 기동함 | URL · "1분마다 자동 갱신됩니다" · `/dashboard serve stop {포트}` (끝나면 서버를 끄라는 안내) · 최초 1회라면 `Bash(python3 -m http.server:*)`·`Bash(open:*)` 등을 허용 목록에 추가하면 다음부터 권한 프롬프트가 사라진다는 안내 한 줄 |
...
<script>
(function(){
  var POLL_INTERVAL_MS = 60000;         // 로컬 파일 폴링 주기(1분). 포커스·가시성 전환에서
                                         // 즉시 한 번 더 폴링하므로(아래 리스너) 사람이 보는
                                         // 순간의 지연은 이 값과 무관하다 — 사용자 요청.
