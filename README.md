...
- **`/dashboard`** — 전체 경로 작업의 진행 상황을 프로젝트 로컬 `.claude/dashboard.html`
  ...
  로 지움), 플로팅 압축 뷰에서는 진행중인 단계의 상세만 노출됩니다. 「구현」 매크로 단계에는
  `impl set "<작업1|...>"`로 세부 작업 목록을, `impl <k> <done|active|wait>`로 항목별 상태를
  갱신하는 전용 패널이 붙으며, 여러 서브에이전트가 동시에 `active`일 수 있습니다. 목록을
  정의한 적이 없으면 카드 전체가 CSS로 자동 숨겨져 화면이 기존과 동일하고, 상태는 메인
  세션이 서브에이전트를 디스패치하거나 결과 보고를 받는 시점에만 갱신되므로 서브에이전트
  내부의 실시간 진행률을 보여주는 것은 아닙니다.
